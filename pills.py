...
class PillsDataset(utils.Dataset):
	...
	def load_mask(self, imageID):
		...
		# is all 1's
		return (
			masks.astype("bool"), np.ones((masks.shape[-1],), dtype="int32"))


if __name__ == "__main__":
	# construct the argument parser and parse the arguments
	ap = argparse.ArgumentParser()
	ap.add_argument("-m", "--mode", required=True, help="either 'train',\
	'predict', or 'investigate'")
	ap.add_argument("-w", "--weights", help="optional path to pretrained weights")
	ap.add_argument("-i", "--image", help="optional path\
	to input image to segment")
	args = vars(ap.parse_args())

	# check to see if we are training the Mask R-CNN
	if args["mode"] == "train":
		# load the training dataset
		trainDataset = PillsDataset(IMAGE_PATHS, ANNOT_PATH, CLASS_NAMES)
		trainDataset.load_pills(trainIdxs)
		trainDataset.prepare()

		# load the validation dataset
		valDataset = PillsDataset(IMAGE_PATHS, ANNOT_PATH, CLASS_NAMES)
		...
		model.train(
			trainDataset, valDataset, epochs=10,
